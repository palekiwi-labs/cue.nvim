  ...
  pickers.new({}, {
    ...
    attach_mappings = function(prompt_bufnr, map)
      ...
      local function activate()
        ...
        if not output then
          vim.notify("Cue context pin failed, not activating " .. identity .. ": "
      ...
      local function pin()
        ...
        if operation == "unpin" and core.context_is_active(status, entry.value) then
          vim.notify("Cue: " .. identity .. " is the active context and stays pinned;"
            .. " switch away before unpinning", vim.log.levels.WARN)
          return
        end
