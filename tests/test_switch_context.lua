...
check("emits `cue context switch <slug>`", function()
	assert_list_equal(core.switch_context_argv("auth-login"), { "cue", "context", "switch", "auth-login" })
end)
...
check("forwards dir as -C before the slug", function()
	assert_list_equal(
		core.switch_context_argv("auth-login", { dir = "/repo" }),
...
check("forwards store as --store before the slug", function()
	assert_list_equal(
		core.switch_context_argv("auth-login", { store = "/store" }),
...
check("forwards dir and store together", function()
	assert_list_equal(
		core.switch_context_argv("auth-login", { dir = "/repo", store = "/store" }),
